...
/* the destructor for the format factory deletes any formats that are stored in
   the factory*/
FormatFactory::~FormatFactory()
{
...
bool FormatFactory::checkOut(Media* check_out_media, const char& type_ch)
{
    ...
    cout << "incorrect format type" << endl;
    return false;
}
//--------------------------------------------------------------------------
// returnMedia
/* uses the formats within the factory to call returnMedia which
   ...
   number of copies of a book in the library, but if a digital copy is
   returned then we don't need to do much*/
bool FormatFactory::returnMedia(Media* return_media, const char& type_ch)
{
    //if the given format exists
    if (format_factory_[hash(type_ch)] != nullptr) {
...
int FormatFactory::hash(const char& ch) const
{
    return ch - 'A';
}
