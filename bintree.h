...
//			this implementation. This tree stores NodeData objects.
//This 			means that the tree does not need to know what type of data it is 			storing
//and can be simply changed by changing the NodeData class. 			use insert() to add
//data to the tree and use retrieve to find 			a specified NodeData in the tree.
//The tree uses nodes to store 			its data which have left and right pointers. left
//of a node is 			any data less than that node and right of any node is all data
...
//		of this being a Binary Search Tree, instead it is treated as just
//a 		binary tree
