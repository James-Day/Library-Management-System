...
void Manager::inputMediaFile(string infile) {
    ...
    for (;;) {
        media_in_file >> media_type; // read the type of media
        if (media_in_file.eof())
            return; // stop reading if no more data

        // creates a new media given the type of media
        Media *created_media =
            mediaFactory_.createMedia(media_type, media_in_file);
        // if media was created
        if (created_media != nullptr) {
            library_.insert(created_media, media_type);
        } else { // used to read to end of line holding size
            cout << "INPUT MEDIA ERROR: '" << media_type
                 << "' is not a valid media type" << endl;
...
void Manager::inputPatreonFile(const string infile) {
    ...
    while (!patron_in_file.eof()) {
        ...
        if (new_patron->create(patron_in_file)) {
            if (!patrons_.retrievePatron(new_patron->getID(), found_patron)) {
                patrons_.Insert(new_patron);
...
void Manager::inputTransactions(const string infile) {
    ...
    while (true) { // runs till end of file
        ...
        if (!commandFactory_.executeCommand(this, command_type,
                                            command_in_file)) {
