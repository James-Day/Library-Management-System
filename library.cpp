...
/* constructor for the library creates binary search trees to store each type
   ...
   in the library.*/
Library::Library() {
    for (int i = 0; i < kMaxMedia; i++) {
        containers_[i] = nullptr;
    }
    // container for Children Books
    containers_[hash('C')] = new BinTree("Childrens Books");
    // container for Fiction Books
    containers_[hash('F')] = new BinTree("Fiction Books");
    // container for Periodical Books
    containers_[hash('P')] = new BinTree("Periodicals");
}
...
bool Library::insert(Media *insert_media, char media_type) {
    return containers_[hash(media_type)]->insert(insert_media);
}
...
Media *Library::checkOutMedia(const Media *check_out_media, char media_type) {
    Media *found = nullptr;
    if (containers_[hash(media_type)]->retrieve(*check_out_media, found)) {
        return found;
    }
    // book was not found
    return nullptr;
...
bool Library::checkInMedia(Media *check_in_media, char media_type) {
    Media *found = nullptr;
    if (containers_[hash(media_type)]->retrieve(*check_in_media, found)) {
        found->checkIn();
        return true;
    }
...
int Library::hash(const char &type_media) const { return type_media - 'A'; }
